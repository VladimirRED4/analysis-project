...
fn do_unquote(input: &str) -> Result<(&str, String), ParseError> {
    ...
    for c in chars {
        ...
        match (c, escaped_now) {
            ...
            ('"', false) => return Ok((&input[pos..], result)),
            (c, _) => {
                result.push(c);
                escaped_now = false;
            }
...
impl<T: Parser> Parser for List<T> {
    ...
    fn parse<'a>(&self, input: &'a str) -> Result<(&'a str, Self::Dest), ParseError> {
        ...
        while !current_input.is_empty() {
            ...
            match current_input.strip_prefix(']') {
...
impl Parsable for AuthData {
    ...
    fn parser() -> Self::Parser {
        map(take(AUTHDATA_SIZE, stdp::Byte), |authdata| {
            AuthData(authdata.into_boxed_slice())
        })
    }
}
...
mod test {
    ...
    fn test_log_kind() {
        ...
        let auth_data = AuthData(Box::new([
            ...
            ,
        ]));
